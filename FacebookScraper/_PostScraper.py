...
class _PostScraper():        
    ...
    def _post_formalizer(post_object, GMT, cleaning_message=True):
        """
    ...
    def _post_storer(storer, objects, min_date_limit, GMT, max_post_limit=None, cleaning_message=True):
        """
        ...
        if (max_post_limit is None):
            for obj in objects['data']:
                obj = _PostScraper._post_formalizer(obj, GMT, cleaning_message)
    ...
    def get_post_data(engine, target_id, add_field=None, remove_field=None, day_limit=7, count_limit=None, GMT='+7:00', cleaning_message=True):
        """
        ...
        if (status is not None):
            ...
            if (_PostScraper._post_storer(storer=posts, objects=objects, min_date_limit=min_date_limit, GMT=GMT, max_post_limit=max_post_limit, cleaning_message=cleaning_message) is True):
                if len(posts) > 0:
                    while True:
                        ...
                        if (_PostScraper._post_storer(storer=posts, objects=objects, min_date_limit=min_date_limit, GMT=GMT, max_post_limit=max_post_limit, cleaning_message=cleaning_message) is False):
                            logging.info('Scraping %s (%s) posts complete! Total : %i' % (status['name'], status['id'], len(posts)))
                            break
                else:
                    logging.info('Scraping %s (%s) posts complete! Total : %i' % (status['name'], status['id'], len(posts)))
