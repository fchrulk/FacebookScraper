...
class _CommentScraper():        
    ...
    def _comment_formalizer(source, comment_object, GMT, cleaning_message=True):
        """
    ...
    def _comment_storer(source, storer, objects, GMT, max_comment_limit='all', cleaning_message=True):
        """
        ...
        if (max_comment_limit is not 'all'):
            for obj in objects['data']:
                obj = _CommentScraper._comment_formalizer(source, obj, GMT, cleaning_message)
    ...
    def get_comment_data(engine, target_id, add_field=None, remove_field=None, count_limit='all', GMT='+7:00', cleaning_message=True):
        """
        ...
        if (status is not None):
            ...
            if (_CommentScraper._comment_storer(source=status, storer=comments, objects=objects, GMT=GMT, max_comment_limit=max_comment_limit, cleaning_message=cleaning_message) is True):
                if len(comments) > 0:
                    while True:
                        ...
                        while True:
                            try:
                                ...
                                if reload_count == 10:
                                    ...
                                    break
                        if (_CommentScraper._comment_storer(source=status, storer=comments, objects=objects, GMT=GMT, max_comment_limit=max_comment_limit, cleaning_message=cleaning_message) is False):
                            logging.info('Scraping post %s comments completed! Total : %i' % (status['id'], len(comments)))
                            break
                else:
                    logging.info('Scraping post %s comments completed! Total : %i' % (status['id'], len(comments)))
